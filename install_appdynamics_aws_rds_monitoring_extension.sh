#!/bin/sh -eux
#---------------------------------------------------------------------------------------------------
# Install AppDynamics AWS rds monitoring extension from the AppDynamics Community Exchange.
#
# This extension captures statistics for AWS rds instances from Amazon CloudWatch and
...
# [MANDATORY] appdynamics aws rds monitoring extension account parameters.
set +x  # temporarily turn command display OFF.
AWS_ACCESS_KEY_ID="${AWS_ACCESS_KEY_ID:-}"
AWS_SECRET_ACCESS_KEY="${AWS_SECRET_ACCESS_KEY:-}"
set -x  # turn command display back ON.

# [OPTIONAL] appdynamics aws rds monitoring extension install parameters [w/ defaults].
...
appd_aws_rds_extension_release="${appd_aws_rds_extension_release:-2.0.4}"
appd_aws_rds_extension_build="${appd_aws_rds_extension_build:-1602566706}"
...
Usage:
  ...
  [OPTIONAL] appdynamics aws rds monitoring extension install parameters [w/ defaults].
    ...
    [root]# export appd_aws_rds_extension_release="2.0.4"               # [optional] aws rds extension release (defaults to user '2.0.4').
    [root]# export appd_aws_rds_extension_build="1602566706"            # [optional] aws rds extension build (defaults to user '1602566706').

  [OPTIONAL] appdynamics aws rds monitoring extension config parameters [w/ defaults].
    [root]# export appd_aws_rds_extension_config="true"                 # [optional] configure aws rds extension? [boolean] (defaults to 'false').

    NOTE: Setting 'appd_aws_rds_extension_config' to 'true' allows you to perform the Monitoring Extension
    ...
    [root]# export appd_aws_rds_extension_cloudwatch_monitoring="Basic" # [optional] aws cloudwatch monitoring type (defaults to 'Basic').
                                                                        #            valid types:
                                                                        #              'Basic', 'Detailed'
    [root]# export appd_aws_rds_extension_tier_component_id="8"         # [optional] tier component id for aws metric prefix (defaults to '8').
...
if [ -n "$appd_aws_rds_extension_cloudwatch_monitoring" ]; then
  case $appd_aws_rds_extension_cloudwatch_monitoring in
      Basic|Detailed)
        ;;
      *)
        echo "Error: invalid 'appd_aws_rds_extension_cloudwatch_monitoring' type: Must be 'Basic' or 'Detailed'."
        usage
        exit 1
        ;;
  esac
...
if [ ! -d "${appd_aws_rds_extension_monitors_path}" ]; then
  echo "Error: '${appd_aws_rds_extension_monitors_path}' machine agent monitors folder does not exist."
  exit 1
fi

# change directory to the machine agent 'monitors' folder.
cd ${appd_aws_rds_extension_monitors_path}

# download the appdynamics aws rds monitoring extension binary.
rm -f ${appd_aws_rds_extension_binary}
curl --silent --location --remote-name https://www.appdynamics.com/media/uploaded-files/${appd_aws_rds_extension_build}/${appd_aws_rds_extension_binary}
chmod 644 ${appd_aws_rds_extension_binary}

# extract appdynamics aws rds monitoring extension binary.
unzip ${appd_aws_rds_extension_binary}
rm -f ${appd_aws_rds_extension_binary}
chown -R ${appd_machine_agent_user}:${appd_machine_agent_user} .
cd ${appd_aws_rds_extension_folder}
...
if [ "$appd_aws_rds_extension_config" == "true" ]; then
  ...
  cd ${appd_aws_rds_extension_path}

  # set current date for temporary filename.
  curdate=$(date +"%Y-%m-%d.%H-%M-%S")

  # save a copy of the current file.
  if [ -f "${appd_aws_rds_extension_config_file}.orig" ]; then
    cp -p ${appd_aws_rds_extension_config_file} ${appd_aws_rds_extension_config_file}.${curdate}
  else
    cp -p ${appd_aws_rds_extension_config_file} ${appd_aws_rds_extension_config_file}.orig
  fi

  # use the stream editor to substitute the new values.
  sed -i -e "/^metricPrefix: \"Server|Component:/s/^.*$/metricPrefix: \"Custom Metrics|Amazon RDS|\"/" _
  ...
  sed -i -e "/^    awsSecretKey:/s/^.*$/    awsSecretKey: \"${aws_secret_key_escaped}\"/" ${appd_aws_rds_extension_config_file}
  set -x    # turn command display back ON.

  sed -i -e "/^    displayAccountName:/s/^.*$/    displayAccountName: \"${appd_aws_rds_extension_display_account_name}\"/" ${appd_aws_rds_extension_config_file}

  # format aws regions string before substitution. (replace whitespace separated list with quoted, comma-separated list.)
  aws_regions_formatted=$(echo "\"${appd_aws_rds_extension_aws_regions}\"" | sed 's/ /", "/g')
  sed -i -e "/^    regions:/s/^.*$/    regions: \[${aws_regions_formatted}\]/" ${appd_aws_rds_extension_config_file}
  ...
  match=$(awk 'BEGIN{found="false"} {if (/monitoring.eu-west-2/) found="true"} END {print found}' ${appd_aws_rds_extension_config_file})

  # if 'eu-west-2' region endpoint not found, add it underneath 'eu-west-1'.
  if [ "$match" == "false" ]; then
    sed -i -e "/^${eu_west_1}/s/^.*$/${eu_west_1}\n${eu_west_2}/" ${appd_aws_rds_extension_config_file}
  fi

  match=$(awk 'BEGIN{found="false"} {if (/monitoring.eu-west-3/) found="true"} END {print found}' ${appd_aws_rds_extension_config_file})

  # if 'eu-west-3' region endpoint not found, add it underneath 'eu-west-2'.
  if [ "$match" == "false" ]; then
    sed -i -e "/^${eu_west_2}/s/^.*$/${eu_west_2}\n${eu_west_3}/" ${appd_aws_rds_extension_config_file}
  fi

  # add 'us-east-2' as a region endpoint. (it is curently missing in release 2.1.3.)
  ...
  match=$(awk 'BEGIN{found="false"} {if (/monitoring.us-east-2/) found="true"} END {print found}' ${appd_aws_rds_extension_config_file})

  # if 'us-east-2' region endpoint not found, add it underneath 'us-east-1'.
  if [ "$match" == "false" ]; then
    sed -i -e "/^${us_east_1}/s/^.*$/${us_east_1}\n${us_east_2}/" ${appd_aws_rds_extension_config_file}
  fi
